...
class PoseDynamicsFactor(gtsam.CustomFactor):
    """A factor implementing rigid body dynamics between two poses, given a linear and angular velocity."""
    ...
    def __init__(
        ...
        """Creates a new PoseDynamicsFactor instance.

        GTSAM has a very particular interface for `error_func` that leads to some strange choices here.
        See: https://github.com/borglab/gtsam/blob/develop/python/CustomFactors.md for more information.

        In particular, we need to pass an error function to super().__init__ that takes (only) a gtsam.CustomFactor, a
        Values object, and optionally a list of Jacobians. That means we can't take other arguments like `dt` directly.

        To work around this, we create a closure of the parameterized self.error_func that captures `dt` and then pass
        that closure to the super() constructor. This is a bit of a hack, but it works.
        ...
        Args:
            ...
            vel_frame: The frame in which the linear velocity is expressed. Can be 'world' or 'body'.
        """
        ...
        super().__init__(
            ...
            partial(self.error_func, dt=dt, vel_frame=vel_frame),
        )
        self.dt = dt
    ...
    def error_func(
        ...
        Takes a factor instance and set of values, and computes the residual between an Euler approximation of the
        dynamics and the next pose. Optionally takes a list of Jacobians that it overwrites in place. We use a hack to
        allow a `dt` parameter to be passed to the factor constructor, which is then captured in the closure for this
        function.
        ...
        Args:
            ...
            dt: The time step for dynamics (not optional, will always get passed via the closure in the constructor).
            vel_frame: The frame in which the linear velocity is expressed. Can be 'world' or 'body'.
        ...
        if H:
            ...
            if vel_frame == "world":
                ...
                # A dirty hack
                # create a trivial rotation transform that can be used to rotate the linear vel into the body frame.
            ...
            pred_pose = pose1 * gtsam.Pose3().Expmap(np.concatenate([dt * ang_vel1, dt * vel1]))
...
class ConstantVelocityFactor(gtsam.CustomFactor):
    ...
    See the docstring for `PoseDynamicsFactor` for more information on the strange interface for `error_func` in GTSAM.
    """
    ...
    def error_func(self, this: gtsam.CustomFactor, v: gtsam.Values, H: Optional[List[np.ndarray]] = None) -> np.ndarray:
        """Returns the difference between two velocities, and optionally computes the Jacobians."""
...
class KeypointProjectionFactor(gtsam.CustomFactor):
    """Implements the KeypointProjectionFactor.

    Computes the difference between a keypoint measurement and the projection of the corresponding body-frame point into
    the camera frame. We reimplement this in Python because the GTSAM ProjectionFactors de facto assume the pose
    variable is camera-to-body, not body-to-camera.
    """
    ...
    def __init__(
        ...
        camera_pose: Optional[gtsam.Pose3] = None,
    ) -> None:
        """Initializes the factor with the given keys, noise model, and problem data.

        See the docstring for `PoseDynamicsFactor` for more info on the strange interface for `error_func` in GTSAM.
        ...
        Args:
            ...
            camera_pose: The pose of the camera in the world frame. Defaults to the identity.
        """
        super().__init__(
            noise_model,
            [body_pose],
            partial(
                ...
                camera_pose=camera_pose if camera_pose is not None else gtsam.Pose3(),
            ),
        )
        self.pixel = None
    ...
    def error_func(
        ...
        Returns the difference between the keypoint measurement and the projection of the corresponding body-frame point
        into the camera frame, and optionally computes the Jacobians.
        ...
        Returns:
            The difference between the keypoint measurement and the projection of the corresponding body-frame point
        ...
        if H:
            ...
            point_camera_frame = body_pose.transformFrom(point_body_frame, dpc_dpose, dpc_dpoint)

            # Create camera and project point onto image plane.
            camera = gtsam.PinholeCameraCal3_S2(camera_pose, camera_intrinsics)
            pixel = camera.project(point_camera_frame, dproj_dpose, dproj_dpoint, dproj_dcal)
